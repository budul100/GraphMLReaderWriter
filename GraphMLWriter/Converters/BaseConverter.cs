...
namespace GraphMLWriter.Converters
{
    internal abstract class BaseConverter<T>
    {
        ...
        public BaseConverter(Type type, KeyConverter keyConverter, keyfortype forType)
        {
        ...
        protected static Func<Type, BaseConverter<edgetype>> GetEdgeConverterGetter(KeyConverter keyConverter)
        {
        ...
        protected static Func<Type, BaseConverter<graphtype>> GetGraphConverterGetter(KeyConverter keyConverter)
        {
        ...
        protected static Func<Type, BaseConverter<nodetype>> GetNodeConverterGetter(KeyConverter keyConverter)
        {
        ...
        protected Func<object, V> GetItemGetter<V, U>(Type type, Func<Type, BaseConverter<V>> converterGetter)
            where U : Attribute
        {
            var properties = type.GetProperties()
                .Where(p => p.GetCustomAttribute(typeof(U)) != null).ToArray();
        ...
        protected Func<object, IEnumerable<V>> GetItemsGetter<V, U>(Type type, Func<Type, BaseConverter<V>> converterGetter)
            where U : Attribute
        {
            var properties = type.GetProperties()
                .Where(p => p.GetCustomAttribute(typeof(U)) != null).ToArray();
        ...
        private IEnumerable<V> GetContents<V>(object input, PropertyInfo property, BaseConverter<V> converter)
        {
